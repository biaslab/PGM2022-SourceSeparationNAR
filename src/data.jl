...
"""
function prepare_data(input_folder::String, output_folder::String; fs::Int64=16000, power_dB::Real=0)
    ...
    if !isdir(output_folder)
        ...
        ProgressMeter.@showprogress for (ind, file) in enumerate(filenames)
            ...
            h5write(last(output_files), "signal", x)
            h5write(last(output_files), "size", collect(size(x)))
...
Base.getindex(x::Data, i::Int) = getindex(x, i, "signal")
...
myread(f::String, item::String) = h5read(f, item)
